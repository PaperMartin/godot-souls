...
var currentRootMotion : Transform
...
func _calculate_movement(delta):
	...
	var velocity : Vector3 = currentRootMotion.origin
...
func _update_root_motion(var delta : float):
	_anim_tree.advance(delta)
	currentRootMotion = _anim_tree.get_root_motion_transform().rotated(Vector3(0,1,0),_body.rotation.y)
